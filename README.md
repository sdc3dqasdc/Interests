...
CMF, TRIX, Williams %R). I include this on purpose: a selection rule I only
ever feed good candidates is a rule I haven't tested.

I don't reuse published ICIRs, they were measured on other universes, eras and
horizons, so I recompute everything on my own universe.
...
in sigma units while the estimator has a 50% breakdown point. One blown-up
print cannot move my clip bounds, which is exactly what a mean/std
...
Pearson IC would largely tell me where the two biggest movers landed. I skip
dates with fewer than 20 eligible names. I write every factor so that **higher
= predicted better**, so a positive IC means it works as stated and a negative
one means it works inverted.
...
a small edge is only worth something spread across many independent bets, which
is why I built the screener as a cross-sectional ranker holding a basket rather
...
touching a real effect**, an asymmetry I want when I'm screening 29 candidates
at once. I also record a decay figure,
...
$\Sigma = \mathrm{diag}(\sigma_i^2)$, exactly the independence assumption §1.4
says I'm violating. I use it anyway because estimating a $5\times 5$ factor
covariance from this sample would add more estimation error than it removes.
...
and, the important part, most are **non-monotone** in the underlying variable:
...
inside an uptrend*, so more 6-month momentum is better only up to about 30%,
and a stock 25% above its 50-day SMA is worse than one sitting on it. Rank IC
cannot express that shape, a factor that is good in the middle and bad at both
ends has an IC near zero, so I validate this score by backtest instead and the
two files disagree by design.
...
to be *in that era*, and $\rho$ drifts. Dividing by the trailing std keeps "+2
sigma" meaning the same thing in 2004 and in 2024, which I need before a fixed
...
my headline result is a negative one. The seed idea, the QQQ/SPY ratio's gap
...
`semis` and `concentration` flip sign between halves, they were never there.
...
**flat above the line and negative below it**, in and out of sample, not a
smooth ladder. Above −0.5σ the Nasdaq wins by roughly the same margin whatever
the reading, so paying turnover to distinguish +0.6 from +2.0 buys me nothing.
Hence a two-state switch rather than a continuous tilt, at a threshold I
deliberately left untuned (Sharpe is 0.61 to 0.63 anywhere from −0.75 to 0.0).
...
transfer, the reading I recommend keeps all eight components. With only about
5.5 years of traded history I don't have the sample to justify trimming, and
